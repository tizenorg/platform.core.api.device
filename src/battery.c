...
int device_battery_get_health(device_battery_health_e *health)
{
	...
	else if (!strncmp(info.health, "Unspecified failure", len))
		*health = DEVICE_BATTERY_HEALTH_UNSPECIFIED_FAILURE;
	else
		*health = DEVICE_BATTERY_HEALTH_UNKNOWN;
...
int device_battery_get_status(device_battery_status_e *status)
{
	...
	if (!strncmp(info.status, "Charging", len))
		...
		*status = DEVICE_BATTERY_STATUS_UNKNOWN;
